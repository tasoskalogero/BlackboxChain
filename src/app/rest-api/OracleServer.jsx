...
async function getDatasetCost(datasetID) {
...
async function getSoftwareCost(softwareID) {
    let accounts = await web3.eth.getAccounts();
    let currentAccount = accounts[9];
...
async function getContainerCost(containerID) {
    let accounts = await web3.eth.getAccounts();
    let currentAccount = accounts[9];

    let ContainerRegistry = initContract(ContainerRegistryJSON);
...
async function getDockerContainerID(containerID) {
    let accounts = await web3.eth.getAccounts();
    let currentAccount = accounts[9];
...
async function runExec(execID, paymentID) {
...
async function createExecInstance(containerID, softwareID, datasetID, userPubKey) {
    ...
    orderReceipt.OrderEvent({fromBlock: latestBlock}, async (error, event) => {
        if (error) {
            ...
            if (event.blockNumber !== latestBlock) {
                ...
                //CHECK IF FUNDS IN ORDER ARE SUFFICIENT
                let swCost = await getSoftwareCost(event.args.softwareID);
                let dsCost = await getDatasetCost(event.args.datasetID);
                let containerCost = await getContainerCost(event.args.containerID);
                ...
                // AMOUNT IN ORDER IS EQUAL WITH THE EXPECTED COST VALUES, THEN SET ORDER AS VALID
                if(expectedCost === orderInfo[4].toNumber()) {
                    let success = await orderReceipt.validateOrder(event.args.orderID, {from: currentAccount});

                    let containerDockerID = await getDockerContainerID(event.args.containerID);

                    //CREATE EXEC INSTANCE
                    let execResult = await createExecInstance(containerDockerID ,event.args.softwareID, event.args.datasetID, userPubKey);
                    ...
                    if (execResult[0] === "FAILURE") {
                        ...
                        //EXECUTE
                        let msg = await runExec(exec_id, event.args.orderID);
                    ...
                    //AMOUNT SENT IN ORDER DOESN'T MATCH WITH THE EXPECTED COST, SO ABORT
                }
            }
        }
    });
...
// let server = app.listen(8081, async () => {
