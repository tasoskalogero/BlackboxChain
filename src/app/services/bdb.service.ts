...
export class BdbService {
    ...
    async createNewSoftware(filename: any, ipfsHash: any, paramType: any, description: any, cost: any) {
        const owner = new bcdb_driver.Ed25519Keypair();
        console.log('Posting software: ' + filename + ' ' + ipfsHash + ' ' + paramType + '' + description + ' ' + cost);
        let asset = {filename: filename, ipfsHash: ipfsHash, paramType: paramType, description: description, cost: cost};
        let metadata = null;
